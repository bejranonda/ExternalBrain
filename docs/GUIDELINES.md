...
**There are TWO generations of scope helper, and the V1 set has no cross-project reach.** `buildKnowledgeWhere` / `buildSessionWhere` / `buildProposalWhere` / `buildRawProjectFilter` predate Phase 4 and resolve visibility on `ownerProjectId` alone. The V1 → V2 migration is **unfinished**: the dashboard, the graph and `exporter.buildRulesBundle` still call V1, so they do not see `scope: 'user'` rows taught in other projects even though retrieval does (KNOWN_ISSUES §0p). Before adding a call site, pick V2; before changing visibility behaviour, check which generation the surface you care about actually uses.
