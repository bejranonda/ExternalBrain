...
18. **DIKW-T stage promotion invariant:** The intelligence lifecycle strictly advances through the **DIKW-T Framework** (*Data → Information → Knowledge → Wisdom + Time*). Raw events (`SessionEvent` — Data) promote to typed summaries (`Session.metadata` — Information) only upon session close; KEA/Hermes synthesis produces atomic `Knowledge` rows; confidence weighting, decay, and reflection synthesis evolve into versioned institutional policy (Wisdom + Time). No code path creates untyped knowledge without provenance.
...
**Adding or touching a monitor/watchdog? Verify what it reads, not just that it fires.** Dispatching it and watching it open an issue proves the plumbing (schedule registered, HTTP works, comparison runs) — it proves nothing about whether the input is the right input. The `prod-drift` watchdog passed that firing test and still polled the **dev** host for months, so the alarm titled "production is behind main" had never read production (`KNOWN_ISSUES §0al`). The check is one line and belongs in the same PR: read the value the monitor reports, independently query the system you believe it watches, and confirm they describe the same thing. Beware the specific trap that hid this one — a monitor pointed at the wrong target often still emits *plausible* alarms, and a plausible alarm agreeing with something you already believe is the weakest possible evidence that it is wired correctly. Where the target is a secret, emit a non-secret discriminator in the alert body so a reader can tell which system was measured without repo-admin access.
  ...
  **The implemented pattern, reusable for any new monitor:** (1) the monitored system *declares its own identity* on the endpoint being polled — `/api/healthz` returns `environment` from `BRAIN_DEPLOY_ENV`; (2) the monitor *asserts* that identity against a readable, non-secret statement of intent (`vars.BRAIN_EXPECT_DEPLOY_ENV`) and **fails red on mismatch**, so pointing it at the wrong box is an error rather than a plausible alarm; (3) an *absent* identity maps to "cannot verify" and suppresses the alarm — fail-safe, never a default that lets an unconfigured host certify itself; and (4) the alert body **names the identity it measured**. Give the declared-identity field a test: it is a contract read from outside the repo, so dropping it otherwise degrades the monitor to silence. Do **not** reuse a pre-existing environment label for (1) without checking its real value on every host — this project's prod `.env` says `ENVIRONMENT=dev`, and reusing it would have made the guard certify production as dev.
...
- **A denied destructive-write command is a fact about the gate, not something a follow-up message unlocks.** Raw destructive SQL against a live database (a `docker exec … psql … DELETE`, not a Prisma migration) goes through the harness's auto-mode classifier, independent of `AGENTS.md §3`'s Prisma-level consent flag. It is deterministic: retrying the identical command, even after the user explicitly says "run it," is denied identically, because the classifier evaluates the call itself, not the surrounding chat. The only unlock is the user exiting auto mode (a distinct harness state). Before any such request: survey and state the exact blast radius (tables, row counts, FK-order — `Project.organizationId`'s `RESTRICT` is a real example of order mattering) so whichever approval path is used is informed. Full incident: `KNOWLEDGE.md §12.36`, `KNOWN_ISSUES.md §0am`, `APPROACH.md §5bx`.
...
- **When two routes share an invariant, check it in one place — or audit every account shape that could violate it.** `forgot-password` and `reset-password` each independently required a `UserCredential` to exist, which was fine for credentials-signup accounts but silently locked out `/api/onboard/claim` accounts (deliberately credential-less) forever — no error, no log line, indistinguishable from "no such user." Prefer making the *reset* flow tolerant (upsert the credential, don't require it) over trying to enumerate and gate every account-creation path that might not set one; new account shapes will keep appearing. See `KNOWN_ISSUES.md §0an`.
...
`packages/core/src/oracle.ts` uses the Anthropic SDK whenever `model.startsWith("claude")` OR `ANTHROPIC_BASE_URL` is set. When routing to an Anthropic-compatible provider (Z.ai GLM, Bedrock, Vertex), pass the provider's own model name (e.g. `glm-5.2`) in `ORACLE_MODEL` — don't rename it to look Claude-ish. Add the price row in `packages/core/src/cost.ts` so cost-cap accounting works.

**The requested model is not the served model (2026-08-18).** Anthropic-compatible gateways **alias silently rather than 404** — measured on the Z.ai Coding Plan endpoint: `glm-5.1`, `glm-5.2` and `glm-5` were all answered by `glm-5.3`, and `claude-haiku-4-5` by `glm-4.7`, each with a 200. Consequences, all silent: the cost ledger prices the model you *asked for*, docs describe a model that never ran, and a `claude-*` "fallback" can be served by a GLM model. **Rule:** `res.model` is the only ground truth. Every Anthropic dispatch site must call `reportServedModel(requested, served)` from `llm.ts`, which warns once per pair. There are **three** such sites — `llm.ts` (1) and `oracle.ts` (2, one non-streaming and one streaming) — and instrumenting only `llm.ts` misses the Oracle entirely, which is the path whose model actually gets aliased. Before adding cross-cutting instrumentation to any LLM path, `grep -n 'messages.create\|messages.stream' packages/core/src/*.ts` and cover every hit; this is the same one-rule-two-implementations class that killed cross-session KEA for eight nights.

**Validate agent-supplied text at the MCP boundary, not after (2026-08-23).** Tools whose fields are written by an LLM get malformed input in a specific shape: a later parameter typed *inside* an earlier field's value, so the tail is stored as text and every parameter after it is dropped — while the call returns a normal success token. Two `Knowledge` rows landed that way, one of them a project decision that silently lost the `decision` tag and therefore its `visibility: "org"`. **Rule:** reject obviously-corrupted text before it is written, with an error that names the likely cause and the consequence; never store it. **And count the doors first** — agent-authored text reaches `Knowledge` through `brain_teach_knowledge` *and* through `brain_report_session_outcome.learnings` (promoted by KEA refine). v2.19.3 guarded one and had to be followed by v2.19.4; both now share `packages/core/src/text-guards.ts`. Where a path cannot reject because it owes the caller a never-block contract, **drop and report** instead: a drop the submitter cannot see is the same silent loss as storing the corruption. Keep the predicate narrow — knowledge rules routinely discuss code, so `Array<string>`, `a < b` and `<Component/>` must pass — and pin that with tests, because a guard that rejects legitimate prose is worse than the bug. Audit an existing corpus with `SELECT id FROM "Knowledge" WHERE rationale LIKE '%</rationale>%' OR rationale LIKE '%<parameter%'`.

**One canonical home per lesson; cross-link, never restate (2026-08-18).** `KNOWN_ISSUES.md` is ~2,300 lines and `APPROACH.md` ~3,000. The reflex to "update the README and all related docs" after every fix is what produced that, and it is now costing more than it returns: a lesson restated in five files is a lesson no reader will find, plus four copies that drift independently — §0ak is the instance where duplicated guidance was silently reverted by a rewrite of one copy. **Rule:** the incident narrative goes in `KNOWN_ISSUES.md` and the transferable method in `APPROACH.md`, each exactly once; everywhere else links to it. A doc that needs the *fact* (a model name, a command, an invariant) states the fact in one line and links for the reasoning. Prefer teaching a durable rule to the Brain (`brain_teach_knowledge`) over adding a paragraph an agent has to grep 3,000 lines to find — that is what the product is for, and this repo should dogfood it.

**Never change a KEA or Oracle model without running the eval harness — on BOTH paths.** `pnpm --filter @brain/worker eval:kea --path both` replays the production prompts over real sessions carrying `learning_captured` events and reports `empty` / `schemaBad` / `parseFail` / p95 per candidate. It exists because a single synthetic payload ranked `glm-5.3` first while, on five real sessions, it returned **zero findings on three of them**. KEA's failure mode is a silent empty extraction — no exception, no failing test, no unhealthy probe — so a model regression is invisible in production until someone notices the Brain stopped learning. Rank by fewest `empty`, then `schemaBad: 0`, then p95; total finding count is the weakest signal (verbose ≠ better), and any model with `callFail > 0` is UNRANKABLE rather than top-ranked.

**Score the path production actually takes.** KEA has two: `mine` (no submitted learnings) and `refine` (judges the learnings an agent submitted). Audited 2026-08-20, **4 of 4 post-upgrade extractions were `refine` and 0 were `mine`** — so the harness, which originally scored only `mine`, measured a path production barely uses while leaving the one it always uses unmeasured. The models tied on quality on `refine` and the winner was the *slower* one there, which the original rationale (fewer output tokens on `mine`) had not predicted. Before trusting any model comparison, check which path the traffic takes: `SELECT payload->>'mode', count(*) FROM "AuditLog" WHERE action='kea.extract_session' GROUP BY 1;`
...
**Changing the embedding model is a data migration, not a config change (2026-08-18).** Vectors from different models are not comparable — measured cosine similarity for the *same sentence* across `gemini-embedding-001` and `gemini-embedding-2-preview` was **−0.024**, i.e. orthogonal, not "slightly different". A partial migration therefore does not degrade retrieval, it destroys it: new query vectors score ~0 against every row still on the old model, so the Oracle returns nothing relevant and **raises no error**. Two mechanics make this safe, and both must stay intact: (a) every row records `embeddingModel`, and the backfill selects `embedding IS NULL OR "embeddingModel" IS DISTINCT FROM $active` so stale rows are re-embedded rather than stranded; (b) `activeEmbeddingModel()` in `embedding.ts` is the single source of truth for "which model is live", used both to embed and to detect staleness — if those two ever diverge the table either never converges or re-embeds itself forever. Watch `remaining` in the `embeddings.backfill` job log: it counts down to 0 as the index converges, and a value stuck across runs means re-embedding is failing, which retrieval cannot tell you. Note `EMBEDDING_MODEL` selects the *primary* Gemini model only when it names one (`gemini-*`); any other value configures the fallback entry alone — it used to configure nothing at all while appearing to work.
