...
**Never change a KEA or Oracle model without running the eval harness.** `pnpm --filter @brain/worker eval:kea` replays the production prompt over real sessions carrying `learning_captured` events and reports `empty` / `schemaBad` / `parseFail` / p95 per candidate. It exists because a single synthetic payload ranked `glm-5.3` first while, on five real sessions, it returned **zero findings on three of them**. KEA's failure mode is a silent empty extraction — no exception, no failing test, no unhealthy probe — so a model regression is invisible in production until someone notices the Brain stopped learning. Rank by fewest `empty`, then `schemaBad: 0`, then p95; total finding count is the weakest signal (verbose ≠ better).
