...
- **An anonymous endpoint that vends a credential needs its own master switch, and the switch defaults off.** Rate limits bound abuse; they do not make the capability optional. `POST /api/onboard/claim` exchanges a voucher for a live MCP token with no session behind it, so it is gated on `AGENTIC_ONBOARDING` and only the literal `"true"` opens it — `"1"`, `"yes"` and `" true"` all stay shut, because those are the values an operator guesses and a guess must not open a bearer-vending endpoint. Reusing an existing knob would have been tempting (`REGISTRATION_REQUIRES_VOUCHER` is *right there*), but it answers a different question: "may strangers create accounts" is not "may strangers mint tokens", and collapsing them removes the operator's ability to run open signup without headless token issuance. When a new capability changes what a *leaked* existing secret is worth — here, the voucher stops being an account stub and becomes bearer-equivalent — that is the signal it needs its own switch and its own paragraph in `docs/SECURITY.md`.
