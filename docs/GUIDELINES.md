...
| E2E | `apps/web/e2e` | 27 specs covering every surface + tokens + onboarding + tweaks + i18n + a11y + responsive + mobile-overflow + streaming. Run the full suite locally with `pnpm --filter @brain/web e2e` (dev stack must be up). The **anonymous subset** (`welcome-public-urls`, `healthz`, `security`) runs automatically in CI via the `onboarding-e2e` gate whenever an onboarding/unauth surface changes — see [CICD.md](./CICD.md). |
