...
12. Retrieval scoring weights (`kra.ts` `WEIGHTS`) require a benchmark run before change — export a fixture and run `pnpm --filter @brain/core run benchmark:retrieval fixture.json` (NDCG@5, KRA vs cosine); record before/after in `docs/VALIDATION.md` in the same PR.
