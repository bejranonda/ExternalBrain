...
**Validate agent-supplied text at the MCP boundary, not after (2026-08-23).** Tools whose fields are written by an LLM get malformed input in a specific shape: a later parameter typed *inside* an earlier field's value, so the tail is stored as text and every parameter after it is dropped — while the call returns a normal success token. Two `Knowledge` rows landed that way, one of them a project decision that silently lost the `decision` tag and therefore its `visibility: "org"`. **Rule:** reject obviously-corrupted text before it is written, with an error that names the likely cause and the consequence; never store it. Keep the predicate narrow — knowledge rules routinely discuss code, so `Array<string>`, `a < b` and `<Component/>` must pass — and pin that with tests, because a guard that rejects legitimate prose is worse than the bug. Audit an existing corpus with `SELECT id FROM "Knowledge" WHERE rationale LIKE '%</rationale>%' OR rationale LIKE '%<parameter%'`.
