...
|---|---|---|---|
| 1 | `brain_start_session` | ONCE at the start of a coding task. Accepts optional `projectName` to file the session under a project (creating it on demand). | `{ sessionId, startedAt }` — save and pass everywhere |
...
| 6 | `brain_report_session_outcome` | AFTER user accepts/rejects | `{ sqs, queued }` |
