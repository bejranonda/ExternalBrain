...
| **`queue_status().pending_work_units == 0`** before querying after an ingest | **Not adopted yet.** No `pending_work_units` gate exists, and no client can block on "brain is consolidated" — a close is enqueued and the caller returns immediately, so knowledge from the session just closed is not yet retrievable. The nearest built surface is `GET /api/admin/queue-health`, which is admin-only and reports dead-letter depth + 24 h failure counts, i.e. *did the pipeline break*, not *has it caught up*. This row read as though the endpoint shipped until 2026-08-30 (`KNOWN_ISSUES §0at`). |
