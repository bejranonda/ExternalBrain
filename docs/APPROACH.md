...
**A sweep guards the literals it lists, not the property it advertises (2026-08-15).** `install-command-single-source.test.ts` opens by declaring that it checks "only one place is *able* to construct the command". It does not: it greps for two literals, `/api/onboard.sh | bash` and `/api/onboard.ps1 -UseBasicParsing | iex`. A new agent-facing document that hand-wrote `claude mcp add …` — a different command shape for the same job — passed the guard untouched, and would have shipped an install that skipped the skill install and the smoke test the real installer performs. The gap is not fixable by adding `mcp add` to the pattern list, because `skill-template.ts` legitimately *mentions* that command in prose; a guard keyed on literals cannot separate constructing from quoting. The durable lesson is narrower than "write a sweep": **state the property in the test's name and then check whether the detector can actually decide it** — when it can't, say so in the file rather than letting the docstring over-promise. Tracked in `KNOWN_ISSUES §0ao`.

### 2.6f Shipped, rendered, and discoverable are three different claims (2026-08-15)
The prompt-install tab was merged, deployed, and verified live — the endpoint returned 200, the rendered document was read end to end, the MCP half was exercised against production, and every gate was green. It was still, for practical purposes, not shipped: the unselected tab used a ghost style with no border or background, so the second install route read as a caption next to a button (`GUIDELINES §7c`). The defect survived because every check answered a question one step short of the one that mattered — *does the endpoint respond*, *does the component render*, *do the selectors resolve* — and none asked *would a user find this*.

The correction is not "add screenshot tests to everything". It is to **name which claim your evidence supports** and notice when the gap between claims is where the user lives. A curl proves reachable. A render test proves the string exists. Only a human eye (or a diffed screenshot) proves discoverable, and for a *user-facing entry point* that is the only claim worth making. Where the loop cannot include an eye, say so in the test plan rather than letting "verified" cover the distance — the operator caught this one from a screenshot in about ten seconds, which is roughly the cost of asking for it.
...
**The commit point is the boundary, not the feature (2026-08-15).** Once a handler has written the row the caller needs an id for, *everything after it is fail-soft* — not just the parts that look optional. `brain_start_session` learned this twice: `relevantKnowledge` and `openActionItems` were each written fail-soft on purpose, then a later change added an unguarded project lookup after `session.create()`. A DB blip there returns an error to a caller whose session row already exists, so the session can never be closed and `brain_report_session_outcome` never runs — the exact unclosed-session failure the whole loop exists to prevent, produced by a query whose only job was to decorate the response. Ask of any post-write statement: *if this throws, does the caller still learn what was created?* If not, it is fail-soft or it moves above the write.
