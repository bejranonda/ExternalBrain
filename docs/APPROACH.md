...
2. **Find the structural gap, not the feature gap.** Built-in memory is **siloed — per tool, per project, and per person**, a **black box**, and **vendor-locked**. Those aren't features a vendor ships next quarter — they're consequences of *where the memory lives*. A structural differentiator (cross-tool, cross-project, cross-team, inspectable, self-hosted, owned) doesn't evaporate when the incumbents add a feature — and the cross-project/cross-team reuse (user/project/team/org scopes) is the enterprise problem the platform was built for.
