...
The cross-platform challenge makes this more valuable, not less. The correct config file path differs between Claude Code (`~/.claude.json`), Cursor (`~/.cursor/mcp.json` or `<repo>/.cursor/mcp.json`), Windsurf (`~/.codeium/windsurf/mcp_config.json`), and Claude Desktop (platform-specific). The shell syntax for a one-liner differs between bash and PowerShell. The wizard handles that asymmetry once, centrally, so each operator gets the right command for their environment without reading a compatibility matrix. The installers (`/api/onboard.sh`, `/api/onboard.ps1`) apply the same principle at the script level: the operator runs one command; the script handles the path, the `claude mcp add` invocation, and the SKILL.md download for their OS.
