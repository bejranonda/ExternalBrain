...
| **The V1 scope helpers never got #174's cross-project reach *for project-scoped calls* — so the default rules export is not the rule set you are served.** Two different things are called "scope" here and the distinction is the whole point: the persisted `Knowledge.scope` (`user` / `project` / `global`) and the request-level `DataScope` (`project` / `all`). `buildKnowledgeWhere` (V1) is live at **7 call sites** — dashboard, graph, `exporter.buildRulesBundle`. With `DataScope: "all"` it returns everything the caller owns, so it *does* reach across projects there. The gap is `DataScope: "project"`, where it resolves as `ownerProjectId = active OR (ownerProjectId IS NULL AND ownerUserId = caller)` and never consults `Knowledge.scope` — so it cannot do what V2's opt-in does: filter to the active project **while still** admitting the caller's `scope: 'user'` rows from elsewhere. Retrieval (`kra.ts`/`oracle.ts`, V2 + opt-in) *does* see those rows. **Concrete symptom:** `buildRulesBundle` defaults to `DataScope: "project"` and the route only passes `"all"` when `?scope=all` is present — so the **default** "Download rules bundle" omits `scope: 'user'` rules the Brain is actively injecting into your sessions. Adding `?scope=all` does return them, which is why this reads as a confusing inconsistency rather than an outright absence. Dashboard and graph diverge the same way in their project-scoped default. This is the unfinished half of the Phase-4 V1→V2 migration (`scope-filter.ts` says V2 "replaces `buildRawProjectFilter` for callers that have been migrated"); some callers were migrated, these were not. **Not fixed here** — it changes what three user-facing surfaces show, which is a product call, not a correctness one. Options: migrate the three call sites to `buildKnowledgeWhereV2` with `includeUserScopeAcrossProjects: true`, or add the same opt-in to V1. Found 2026-08-01 by an independent review of PRs CodeRabbit did not actually read. | `packages/core/src/scope-filter.ts` (V1), `exporter.ts:95`, `api/dashboard/route.ts:25`, `api/graph/route.ts:52` | open — product call |
