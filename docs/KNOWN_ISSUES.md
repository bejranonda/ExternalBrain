...
| **The V1 scope helpers never got the #174 cross-project reach — so the rules you export are not the rules you are given.** `buildKnowledgeWhere` (V1) is still live at **7 call sites**: the dashboard, the graph, and `exporter.buildRulesBundle`. It resolves visibility as `ownerProjectId = active OR (ownerProjectId IS NULL AND ownerUserId = caller)` and never consults `scope`, so a `scope: 'user'` rule taught from inside another project is invisible to it. Retrieval (`kra.ts`/`oracle.ts`, V2 + opt-in) *does* see those rows. **Concrete symptom:** `buildRulesBundle` defaults to `dataScope: "project"`, so "Download rules bundle" on the Skills surface omits rules the Brain is actively injecting into your sessions — the exported set disagrees with the served set. Dashboard and graph counts diverge the same way, less visibly. This is the unfinished half of the Phase-4 V1→V2 migration (`scope-filter.ts` says V2 "replaces `buildRawProjectFilter` for callers that have been migrated"); some callers were migrated, these were not. **Not fixed here** — it changes what three user-facing surfaces show, which is a product call, not a correctness one. Options: migrate the three call sites to `buildKnowledgeWhereV2` with `includeUserScopeAcrossProjects: true`, or add the same opt-in to V1. Found 2026-08-01 by an independent review of PRs CodeRabbit did not actually read. | `packages/core/src/scope-filter.ts` (V1), `exporter.ts:95`, `api/dashboard/route.ts:25`, `api/graph/route.ts:52` | open — product call |
