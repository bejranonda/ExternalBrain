...
| **`deploy-backup-1` unhealthy for 10+ days (as of 2026-06-18).** `docker compose ps` shows the rclone off-host backup sidecar as `Up N days (unhealthy)`. On-host pg_dump backups (`deploy-db-1`) continue running normally, but the off-host replication copy is likely stale or failing silently. Symptom appeared before PR #78; unrelated to the new registration/org features. Investigate with `docker compose -f deploy/docker-compose.yml logs deploy-backup-1 \| tail -50`. Common cause: rclone config missing or stale R2/S3/B2 credentials. Recovery: run `bash scripts/setup-backup-replicate.sh` with current bucket credentials, then `./scripts/deploy.sh` to reload. | `deploy/docker-compose.yml`, `scripts/setup-backup-replicate.sh` | operator action — investigate rclone config |
