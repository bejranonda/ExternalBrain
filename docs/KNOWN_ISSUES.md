...
| Issue | Where | Fix by |
|---|---|---|
| **AI-translated TH / DE strings unreviewed by native speakers.** `oracle.empty_hint` and `app.ingest_ok_zero` in `apps/web/lib/brain/i18n.ts` are AI-generated translations from an early PR (i18n restore after the brief EN-only cycle). **Extended 2026-08-03/04 (v2.8.0, v2.10.2):** the whole `decisions` section (9 keys) and `oracle.tagline` were AI-authored in both locales when the type lock found them missing, then AI-reviewed — a review that fixed a real terminology inconsistency (DE said "Decay" where the dashboard says "Zerfall") and replaced the untranslatable internal term "inject-at-open" with a plain description in both locales. **That review deliberately did NOT touch phrasing or naturalness**, which an AI reviewer cannot judge reliably in a language it does not speak; this row therefore stays open. A native speaker should read the `decisions` block and `oracle.tagline` in both locales. They read naturally but a native-speaker proofread would be welcome before real users hit them in production. Same class of risk as the broader "Thai + German i18n scaffolded but not reviewed by a native speaker" entry in §1; this row tracks the specific new keys. (Severity: low. Workaround: open a PR with a corrected translation if you spot something awkward.) | `apps/web/lib/brain/i18n.ts` | opportunistic |
...
| **Anonymous visitors have no way to switch theme.** `<Tweaks>` — the only component with theme buttons — mounts exclusively in `components/brain/app.tsx`, the signed-in shell. The five anonymous surfaces sharing `<LocalePicker>` (`/`, `/start`, `/signin`, `/welcome`, `/docs`) carry a language switcher but no theme control, so a first-time visitor who dislikes the new light default (`KNOWLEDGE.md §12.33`) has no self-service way to change it before creating an account. Found while verifying the theme-default change was genuinely user-overridable, not while building it — deliberately not fixed in the same PR: expanding UI scope unrequested in the same session flagged for excess PR churn would compound the problem, not fix it (`APPROACH.md §5`). Fix sketch: a small `ThemeToggle` component, positioned like `locale-picker.tsx`, added to the same five pages. | `apps/web/components/brain/tweaks.tsx`, the five `<LocalePicker>` pages | open |
| **A module header can assert a capability the types never permitted, and nothing detects it.** `packages/core/src/org.ts` opened with "Every function accepts a `db` client as the first argument so callers … can supply a transaction client or a mock" — a design rule stated as accomplished fact. All 19 functions took `db: PrismaClient`, and Prisma's `TransactionClient` is `Omit<PrismaClient, ITXClientDenyList>`, so it is not structurally assignable; passing one had never compiled and no caller had ever tried. Reviewers trust the docstring, CI is silent because nothing violates a type, and the cost lands on whoever first *needs* the capability — here `/api/onboard/claim`, whose voucher-burn and token-mint must share a transaction. **Partially fixed (v2.15.0):** a `DbClient` union plus a `canOpenTransaction()` guard now makes `ensurePersonalOrg` and `ensureDefaultProject` genuinely transaction-safe, and the header was rewritten to name *which two* functions qualify instead of claiming all of them. The other 17 still require a full client. **The class stays open:** no check exists that a capability claimed in prose has a caller exercising it. Grep for confident header comments in `packages/core/src/*.ts` when you touch them. | `packages/core/src/org.ts`, `docs/APPROACH.md §2.6c` | class open; instance fixed (v2.15.0) |
...
| **`/welcome` has two jobs and only one of them is still its own.** With `/` now a public landing and `/start` the voucher front door, the public funnel is `/` → `/start` → `/welcome` → `/docs`. `/welcome`'s first two steps — pick an AI tool, copy an install command — are done better by `/start` (for voucher holders) and `/settings/tokens` (for account holders); it shows a **placeholder** token and tells you to go elsewhere for a real one. What it uniquely has is the part nothing else does: the live first-session poll with the 90-second and 5-minute stuck-state diagnostics built for the 60-second promise. **Suggested consolidation:** re-point `/welcome` at that job — the "did it actually work?" destination that `/start` and the installers send people to *after* install — and drop its tool picker, rather than leaving a second entry point competing with `/start`. Not done here because it is a UX change with its own e2e surface (`welcome-public-urls.spec.ts`), not a bug fix to smuggle into a landing-page PR. | `apps/web/components/brain/welcome-flow.tsx`, `apps/web/app/start/page.tsx` | open |
| **The onboarding explanation surface is now ~19 documents and still has no single generator.** `/start` fixed *discoverability* — there is finally one URL to print on a voucher card — but not duplication. The install command and the setup narrative still exist independently in the repo tutorials (`docs/tutorials/00-quick-start.{md,th.md,de.md}`), the handout HTML page, three PDFs, `/welcome`, `/docs`, and the agent bootstrap document. `install-command-single-source.test.ts` now prevents any *source file* from hand-building the install command, which is the half that CI can enforce; the markdown and PDF handouts are outside its reach and can drift silently. A PDF handed out in August with a September command produces a confusing failure the user blames on the product. **Fix sketch:** generate the handouts from `/start` at release time, or extend the sweep to parse the tutorial markdown for installer invocations. | `docs/tutorials/`, `apps/web/lib/brain/install-command-single-source.test.ts` | open |
...
| ~~**Oracle's "copy answer" gives no feedback and throws on a non-secure origin.**~~ **Fixed (v2.11.0).** `onCopy` now returns whether the write landed and `TurnView` owns a per-turn `copyState`, rendering `Copied` / `Clipboard unavailable`. New `oracle.copied` + `oracle.copy_unavailable` keys in all three locales. **The first attempt reached for `common.copied`, which does not exist** — `copied` lives under `sessions` and `welcome` — and `translate()` would have rendered the literal string `common.copied` to users. Caught by grepping for the key rather than assuming it. Last unhardened clipboard call site. | `oracle.tsx:203` is a bare `void navigator.clipboard.writeText(turn.answer)` — no guard, no `copied` state. It is the only remaining unhardened clipboard call site after v2.8.0 (`token-install-wizard`, `skills` ×2, `agent-prompts-card` and now `settings/org` all handle it). Needs a `copied` prop threaded through `TurnView`. | `apps/web/components/brain/oracle.tsx` | open |
