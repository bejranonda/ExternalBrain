...
| **AI-translated TH / DE `/docs` body content unreviewed (#59).** The in-app docs prose (`DOCS_TH` / `DOCS_DE` in `docs-content.ts`) is the largest single block of AI-generated translation in the app — nine concept pages with multi-paragraph bodies. It reads plausibly and keeps technical terms in Latin (Brain/Skill/Oracle/CORS/JWT…), but long-form explanatory prose is exactly where machine translation drifts. A native-speaker proofread is wanted before heavy TH/DE docs traffic; per-slug EN fallback means any page can be reverted to EN by deleting its localized entry. English is authoritative and unchanged. (Severity: low. Workaround: PR a corrected translation, or remove the localized page to fall back to EN.) | `apps/web/lib/brain/docs-content.ts` | opportunistic |
