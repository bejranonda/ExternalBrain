...
| **`e2e/welcome-public-urls.spec.ts` lost its only anon-surface MCP-URL assertion, with no replacement.** Removing `/welcome`'s install snippet (above) also removed the last anonymous page that renders an env-resolved MCP URL — the quick-start tutorial shows `https://<your-brain>/...` placeholders from static markdown, not a real host. The #293 class (unreachable `${hostname}:3100` URLs) is still guarded by `lib/brain/public-urls.test.ts` (source-level, unconditional) and `e2e/tokens.spec.ts` (authed token wizard), but there is now a real gap: nothing anonymous exercises URL resolution end-to-end in a browser. **Fix sketch:** either accept the gap (the source-level test is the one that actually generalizes, per `APPROACH.md §5q`) or add a thin anon spec against `/api/onboard/agent.md`, the one remaining public endpoint that resolves a real host. Left open rather than silently dropped — removed with an explanatory comment in the spec file, not deleted quietly. | `apps/web/e2e/welcome-public-urls.spec.ts` | open |
