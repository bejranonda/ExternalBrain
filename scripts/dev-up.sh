...
# posture. The `edge` services (Caddy/TLS, Redis, backups) stay off — for a
# public server deploy use `./scripts/deploy.sh` instead.
#
# Assumptions:
#   - Docker Engine + Docker Compose v2 available on PATH.
#   - A `.env` at repo root with at minimum DATABASE_URL and one LLM key
#     (e.g. GOOGLE_GEMINI_API_KEY — free tier — or OPENAI_API_KEY).
#
...
# Stamp the build with the current git version so the web UI footer shows it.
